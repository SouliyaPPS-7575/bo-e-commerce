  ...
  const handleExportExcel = async () => {
    try {
      ...
      const exportData = data.map((user: User) => ({
        ...
        [translate('resources.users.fields.verified')]: user.verified ? translate('yes') : translate('no'),
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, translate('resources.users.name'));
...
const VerifiedField = (record: User) => {
  ...
  const handleVerifiedChange = async (
    ...
    try {
      ...
      notify(translate('user_verified_status_updated', {
      ...
      notify(translate('error_updating_verification_status'), { type: 'error' });
    }
  };
...
const UserList = () => {
  ...
  return (
    ...
    >
      ...
      >
        <Column source='avatar' render={AvatarField} label={translate('resources.users.fields.avatar')} />
        <Column source='full_name' label={translate('resources.users.fields.full_name')} />
        <Column source='username' label={translate('resources.users.fields.username')} />
        <Column source='email' field={BlackEmailField} label={translate('resources.users.fields.email')} />
        <Column source='phone_number' label={translate('resources.users.fields.phone_number')} />
        <Column source='role' label={translate('resources.users.fields.role')} />
        {/* <Column source='verified' render={VerifiedField} label={translate('resources.users.fields.verified')} /> */}

        <Column source='created' field={DateField} label={translate('created')} />
        <Column source='updated' field={DateField} label={translate('updated')} />
        {isAdmin && (
          <Column
...
const ResetPasswordButton = ({ record }: { record: User }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      notify(translate('failed_to_send_password_reset_email', { email: record.email }), {
        type: 'error',
      });
    }
  };
